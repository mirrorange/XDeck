...
use super::runtime::{
    kill_process_identity, lookup_process_identity, wait_for_process_identity_exit,
    ProcessRuntimeIdentity, RunningProcess,
};
use super::{
...
impl ProcessManager {
    ...
    pub(super) async fn stop_instance(&self, id: &str, instance_idx: u32) -> Result<(), AppError> {
        ...
        let had_child = proc.child.is_some();
        let had_supervisor = proc.cancel_tx.is_some();
        ...
        if !had_child && !had_supervisor && pty_session_id.is_none() {
